...
<script>
var animatespinner='<i class="fas fa-spinner fa-spin fa-2x"></i>';var totaltxid=<?php echo $totaltxid?>;async function readmsgfromblockhain(txid,num){num=num+1;document.getElementById("comment"+num).value="Loading...";zilliqa=new Zilliqa.Zilliqa('https://dev-api.zilliqa.com');console.log(txid);txn=await zilliqa.blockchain.getTransaction(txid);msg=txn.receipt.event_logs[0].params[0].value;numofrow=msg.split(/\r\n|\r|\n/).length;if(numofrow<5)numofrow=5;document.getElementById("comment"+num).rows=numofrow;document.getElementById("comment"+num).value=msg;console.log(msg);console.log(txn.senderAddress.toLowerCase());senderAddr=Zilliqa.toBech32Address(txn.senderAddress.toLowerCase());console.log(txn.toAddr.toLowerCase());contractAddr=Zilliqa.toBech32Address(txn.toAddr.toLowerCase());blocknum=txn.receipt.epoch_num;newtab=' target="_blank" rel="noopener noreferrer"';url1="https://myzilliqawallet.com/pm/readmessage.php?txid="+txid;myzilliqawalletlink='<a href="'+url1+'"'+newtab+'>'+txid+'</a>';document.getElementById("txid"+num).innerHTML="Message #"+num+" txid: "+myzilliqawalletlink}window.onload=readallmessage;function readallmessage(){for(i=1;i<totaltxid+1;i++)document.getElementById("comment"+i).style.fontSize="10px";txid=[];<?php for($i=0;$i<$totaltxid;$i++)echo"txid[".$i."]=".'"'.$txid[$i].'"; ';?>for(i=0;i<totaltxid;i++)readmsgfromblockhain(txid[i],i);for(i=1;i<totaltxid+1;i++)document.getElementById("comment"+i).style.fontSize="4px"}$("#comment").on("change keyup paste",function(){})$("#size").on("change keyup paste",function(){size=document.getElementById("size").value;console.log(size);if(size=="Font: 2px"){for(i=1;i<totaltxid+1;i++)document.getElementById("comment"+i).style.fontSize="2px"}if(size=="Font: 4px"){for(i=1;i<totaltxid+1;i++)document.getElementById("comment"+i).style.fontSize="4px"}if(size=="Font: 6px"){for(i=1;i<totaltxid+1;i++)document.getElementById("comment"+i).style.fontSize="6px"}if(size=="Font: 8px"){for(i=1;i<totaltxid+1;i++)document.getElementById("comment"+i).style.fontSize="8px"}if(size=="Font: 10px"){for(i=1;i<totaltxid+1;i++)document.getElementById("comment"+i).style.fontSize="10px"}if(size=="Font: 12px"){for(i=1;i<totaltxid+1;i++)document.getElementById("comment"+i).style.fontSize="12px"}if(size=="Font: 14px"){for(i=1;i<totaltxid+1;i++)document.getElementById("comment"+i).style.fontSize="14px"}if(size=="Font: 16px"){for(i=1;i<totaltxid+1;i++)document.getElementById("comment"+i).style.fontSize="16px"}})function viewpage(){pagenum=document.getElementById("page").value;window.location.replace("https://myzilliqawallet.com/pm/viewmessage.php?page="+pagenum)}
</script>
</html>