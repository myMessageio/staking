...
//if (!$con) 	  die('Could not connect: ' . mysqli_error($con));
  ...
  // exit();
   ...
   //for ($blocknum=2315528; $blocknum<=2315541; $blocknum++)       
    
    for ($blocknum=2315429; $blocknum<=2398188; $blocknum++)  
...
//Array ( [0] => ID [1] => amount [2] => data [3] => gasLimit [4] => gasPrice [5] => nonce [6] => receipt [7] => senderPubKey [8] => signature [9] => toAddr [10] => version )
						...
						//	  print_r($array2);
							...
							//print_r($array2);							 
								 ...
								 $nonce=$array2[4];
								 $receipt=$array2[5];								 
								 $senderpubkey=$array2[6]; // contains 0x..
								 $signature=$array2[7]; 
								 $toAddr=$array2[8];
								 $toAddr="0x".$toAddr;
								 //echo "toADDr=".$toAddr; echo "<br>";	
								 $senderpubkey=substr($senderpubkey, 2); // remove the 0x
								 //echo ($senderpubkey);
								 $senderaddress=getAddressFromPublicKey($senderpubkey);
								 $senderaddresszil=toBech32Address($senderaddress);
							...
							else
							{
									 ...
									 $nonce=$array2[5];
									 $receipt=$array2[6];								 
									 $senderpubkey=$array2[7]; // contains 0x..
									 $signature=$array2[8]; 
									 $toAddr=$array2[9];
									 $toAddr="0x".$toAddr;
									 //echo "toADDr=".$toAddr; echo "<br>";	
							...
							echo "gasprice=".$gasprice;echo "<br>";
						 	echo "senderaddress=". $senderaddresszil;echo "<br>";	
						...
						if ($updatedb)
						{
								...
								" ('$datestr',  $timestamp, '$txid','$message', '$senderaddresszil' ) ";	
								echo $qry;			
								$result = mysqli_query($con,$qry);	
								if (!$result) 
								{	
...
function getAddressFromPublicKey($pubkey)
{
	...
	$hexstr=hexStringToByteString($pubkey); 
	$hash=hash("sha256",$hexstr);
	$address = substr($hash, 24); // slice 24
	$address="0x".$address;
	return ($address);	
}
